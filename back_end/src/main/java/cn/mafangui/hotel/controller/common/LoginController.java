...
public class LoginController {
    ...
    public AjaxResult userLogin(String username, String password,
        ...
        if(StringUtils.isEmpty(username)){
            return ResponseTool.failed("Tên người dùng không được để trống");
        }else if(StringUtils.isEmpty(password)) {
            return ResponseTool.failed("Mật khẩu không được để trống");
        }
        User user = userService.selectByUsernameAndPassword(username,password);
        if(user==null){
            return ResponseTool.failed("Tên người dùng hoặc mật khẩu không chính xác");
        }
